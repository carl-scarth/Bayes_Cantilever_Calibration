...
if __name__ == "__main__":
    ...
    debug = False       # Print output for debugging purposes?
    ...
    xf_trans = normalise_inputs(dt_data.x.to_numpy().reshape(-1,1), x_min = x_min.iloc[1], x_max = x_max.iloc[1])
    x_pred_trans = normalise_inputs(x_pred, x_min = x_min.iloc[1], x_max = x_max.iloc[1])
...
#-------------------------------------------------------------------------------
    ...
    with pm.Model() as calibration_model:
        ...
        theta_rep = tt.tile(theta,n).reshape((n,-1),ndim=2)
        ...
        # Implementation using multivariate normal: This seems less stable than the gp method
...
#------------------------------------------------------------------------------
    ...
    theta_trans = rescale_inputs(theta_post,x_min.iloc[0],x_max.iloc[0])
...
#------------------------------------------------------------------------------
    ...
    # predictions is greater than the number of posterior samples set replace = True)
    N_sub_sam = 50
    ...
    with calibration_model:
        ...
        for i, post_i in enumerate(pred_ind):
            ...
            x_train_i = XT.eval({theta: float(theta_post[post_i,0])})
            cov_train_i = cov_func(x_train_i).eval({sigma_eta : float(sigma_eta_post[post_i,0]), ls: ls_post[post_i,:], sigma_e : float(sigma_e_post[post_i,0])})
            # Evaluate quadrant of covariance matrix associated only with predictions
            x_pred_i = np.concatenate((np.tile(theta_post[post_i,0],(n_pred,1)), x_pred_trans),axis=1)
            cov_pred_i = cov_pred(x_pred_i).eval({sigma_eta : float(sigma_eta_post[post_i,0]), ls: ls_post[post_i,:]}) +np.eye(n_pred)*1e-8
            ...
            cross_cov_i = cov_pred(x_train_i,x_pred_i).eval({sigma_eta : float(sigma_eta_post[post_i,0]), ls: ls_post[post_i,:]})
...
#------------------------------------------------------------------------------
    ...
    for ax6 in axes6:
        ...
        ax6.plot(dt_data.x.values, dt_data.delta.values,"gx",markersize=12, markeredgewidth=2, label="Experimental data")
        ax6.legend()
