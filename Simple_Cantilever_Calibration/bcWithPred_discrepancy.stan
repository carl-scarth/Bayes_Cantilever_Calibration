...
// The main modifications are for making efficient predictions by sampling from closed-form expressions for the posterior predictive distribution, and separation of the emulator/discrepancy terms.
...
functions {
  ...
  matrix cal_pred_disc_cov(int n, int m, int n_pred, int p, int q, matrix xf, row_vector tf, matrix xc, matrix tc, matrix x_pred, row_vector beta_eta, real lambda_eta, row_vector beta_delta, real lambda_delta, real lambda_e){
    ...
    // the covariance matrix is actually m + n + 3*n_pred, however, it is only necessary to evaluate the emulator covariance at  m + n + 2*n_pred
    // points, as the n_pred discrepancy predictions do not depend upon the emulator covariance
    xt[1:n, 1:p] = xf;
...
model {
  ...
  // As such, an empty matrix of points is passed to the cal_pred_disc_cov as x_pred, with n_pred set to 0.
