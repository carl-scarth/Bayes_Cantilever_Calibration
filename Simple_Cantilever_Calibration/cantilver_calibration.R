  ...
  setwd("C:/Users/cs2361/Documents/Gaussian Processes")
  ...
  # First we set up the parameters -- the number of beams that we have "observed" 
  # (here -- it is 1), number of data points observed and the number of simulations.
  ...
  # load "true" experimental strain gauge data. Here, we just use the final run.
  ...
  # summarize results to check convergence
  ...
  # Generate plots of samples of the calibrated Gaussian process predictive model
  dev.new(noRStudioGD = TRUE) # plot in new window
  ...
  # plot each of the indexed samples
  ...
  lines(x_plot,defplot,"lwd"=2,"col"="blue")
  # plot legend
  legend(x = "topright",legend = c("calibrated model","true response","observed response"),col=c("magenta","blue","red"),pch=c(NA,NA,4),lty = c(1,1,NA),lwd = c(2,2,2))
  ...
  dev.new(noRStudioGD = TRUE)
  plot(1, type="n", main="Quantiles and Median", xlab="x (m)", ylab="displacement (m)", xlim=c(0, L), ylim=c(min(f_trans), max(f_trans)))
  ...
  # plot legend
  legend(x = "topright",legend = c("median","10th-90th percentile","20th-80th percentile","30th-70th percentile","40th-60th percentile","true response","observed response"),col=c("red",c_light,c_light_highlight,c_mid,c_mid_highlight,"blue","red"),pch=c(NA,NA,NA,NA,NA,NA,4),lty = c(1,1,1,1,1,1,NA),lwd = c(2,2,2,2,2,2,2))
